-- phpMyAdmin SQL Dump
-- version 5.1.1
-- https://www.phpmyadmin.net/
--
-- Anamakine: 127.0.0.1
-- Üretim Zamanı: 24 Kas 2021, 12:46:40
-- Sunucu sürümü: 10.4.21-MariaDB
-- PHP Sürümü: 7.3.30

SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";
START TRANSACTION;
SET time_zone = "+00:00";


/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8mb4 */;

--
-- Veritabanı: `il_ilce`
--

-- --------------------------------------------------------

--
-- Tablo için tablo yapısı `city`
--

CREATE TABLE `city` (
  `id` int(11) NOT NULL,
  `region_id` int(11) NOT NULL,
  `name` varchar(64) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_turkish_ci;

--
-- Tablo döküm verisi `city`
--

INSERT INTO `city` (`id`, `region_id`, `name`) VALUES
(1, 1, 'ADANA'),
(2, 7, 'ADIYAMAN'),
(3, 3, 'AFYONKARAHİSAR'),
(4, 2, 'AĞRI'),
(5, 5, 'AMASYA'),
(6, 6, 'ANKARA'),
(7, 1, 'ANTALYA'),
(8, 5, 'ARTVİN'),
(9, 3, 'AYDIN'),
(10, 6, 'BALIKESİR'),
(11, 6, 'BİLECİK'),
(12, 2, 'BİNGÖL'),
(13, 2, 'BİTLİS'),
(14, 5, 'BOLU'),
(15, 1, 'BURDUR'),
(16, 6, 'BURSA'),
(17, 6, 'ÇANAKKALE'),
(18, 4, 'ÇANKIRI'),
(19, 5, 'ÇORUM'),
(20, 3, 'DENİZLİ'),
(21, 7, 'DİYARBAKIR'),
(22, 6, 'EDİRNE'),
(23, 2, 'ELAZIĞ'),
(24, 2, 'ERZİNCAN'),
(25, 2, 'ERZURUM'),
(26, 4, 'ESKİŞEHİR'),
(27, 7, 'GAZİANTEP'),
(28, 5, 'GİRESUN'),
(29, 5, 'GÜMÜŞHANE'),
(30, 2, 'HAKKARİ'),
(31, 1, 'HATAY'),
(32, 1, 'ISPARTA'),
(33, 1, 'MERSİN'),
(34, 6, 'İSTANBUL'),
(35, 3, 'İZMİR'),
(36, 2, 'KARS'),
(37, 5, 'KASTAMONU'),
(38, 4, 'KAYSERİ'),
(39, 6, 'KIRKLARELİ'),
(40, 4, 'KIRŞEHİR'),
(41, 5, 'KOCAELİ'),
(42, 4, 'KONYA'),
(43, 3, 'KÜTAHYA'),
(44, 2, 'MALATYA'),
(45, 3, 'MANİSA'),
(46, 1, 'KAHRAMANMARAŞ'),
(47, 7, 'MARDİN'),
(48, 3, 'MUĞLA'),
(49, 2, 'MUŞ'),
(50, 4, 'NEVŞEHİR'),
(51, 4, 'NİĞDE'),
(52, 5, 'ORDU'),
(53, 5, 'RİZE'),
(54, 6, 'SAKARYA'),
(55, 5, 'SAMSUN'),
(56, 7, 'SİİRT'),
(57, 5, 'SİNOP'),
(58, 4, 'SİVAS'),
(59, 6, 'TEKİRDAĞ'),
(60, 5, 'TOKAT'),
(61, 5, 'TRABZON'),
(62, 2, 'TUNCELİ'),
(63, 7, 'ŞANLIURFA'),
(64, 3, 'UŞAK'),
(65, 2, 'VAN'),
(66, 4, 'YOZGAT'),
(67, 5, 'ZONGULDAK'),
(68, 4, 'AKSARAY'),
(69, 5, 'BAYBURT'),
(70, 4, 'KARAMAN'),
(71, 4, 'KIRIKKALE'),
(72, 7, 'BATMAN'),
(73, 7, 'ŞIRNAK'),
(74, 5, 'BARTIN'),
(75, 2, 'ARDAHAN'),
(76, 2, 'IĞDIR'),
(77, 6, 'YALOVA'),
(78, 5, 'KARABÜK'),
(79, 7, 'KİLİS'),
(80, 1, 'OSMANİYE'),
(81, 5, 'DÜZCE');

-- --------------------------------------------------------

--
-- Tablo için tablo yapısı `district`
--

CREATE TABLE `district` (
  `id` int(11) NOT NULL,
  `city_plate_id` int(11) NOT NULL,
  `name` varchar(64) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_turkish_ci;

--
-- Tablo döküm verisi `district`
--

INSERT INTO `district` (`id`, `city_plate_id`, `name`) VALUES
(1, 1, 'ALADAĞ'),
(2, 1, 'CEYHAN'),
(3, 1, 'ÇUKUROVA'),
(4, 1, 'FEKE'),
(5, 1, 'İMAMOĞLU'),
(6, 1, 'KARAİSALI'),
(7, 1, 'KARATAŞ'),
(8, 1, 'KOZAN'),
(9, 1, 'POZANTI'),
(10, 1, 'SAİMBEYLİ'),
(11, 1, 'SARIÇAM'),
(12, 1, 'SEYHAN'),
(13, 1, 'TUFANBEYLİ'),
(14, 1, 'YUMURTALIK'),
(15, 1, 'YÜREĞİR'),
(16, 2, 'BESNİ'),
(17, 2, 'ÇELİKHAN'),
(18, 2, 'GERGER'),
(19, 2, 'GÖLBAŞI'),
(20, 2, 'KAHTA'),
(21, 2, 'SAMSAT'),
(22, 2, 'SİNCİK'),
(23, 2, 'TUT'),
(24, 3, 'BAŞMAKÇI'),
(25, 3, 'BAYAT'),
(26, 3, 'BOLVADİN'),
(27, 3, 'ÇAY'),
(28, 3, 'ÇOBANLAR'),
(29, 3, 'DAZKIRI'),
(30, 3, 'DİNAR'),
(31, 3, 'EMİRDAĞ'),
(32, 3, 'EVCİLER'),
(33, 3, 'HOCALAR'),
(34, 3, 'İHSANİYE'),
(35, 3, 'İSCEHİSAR'),
(36, 3, 'KIZILÖREN'),
(37, 3, 'SANDIKLI'),
(38, 3, 'SİNANPAŞA'),
(40, 3, 'ŞUHUT'),
(39, 3, 'SULTANDAĞI'),
(41, 4, 'DİYADİN'),
(42, 4, 'DOĞUBAYAZIT'),
(43, 4, 'ELEŞKİRT'),
(44, 4, 'HAMUR'),
(45, 4, 'PATNOS'),
(46, 4, 'TAŞLIÇAY'),
(47, 4, 'TUTA'),
(48, 5, 'GÖYNÜCEK'),
(49, 5, 'GÜMÜŞHACIKÖY'),
(50, 5, 'HAMAMÖZÜ'),
(51, 5, 'MERZİFON'),
(52, 5, 'SULUOVA'),
(53, 5, 'TAŞOVA'),
(54, 6, 'AKYURT'),
(55, 6, 'ALTINDAĞ'),
(56, 6, 'AYAŞ'),
(57, 6, 'BALA'),
(58, 6, 'BEYPAZARI'),
(59, 6, 'ÇAMLIDERE'),
(60, 6, 'ÇANKAYA'),
(61, 6, 'ÇUBUK'),
(62, 6, 'ELMADAĞ'),
(63, 6, 'ETİMESGUT'),
(64, 6, 'EVREN'),
(65, 6, 'GÖLBAŞI'),
(66, 6, 'GÜDÜL'),
(67, 6, 'HAYMANA'),
(68, 6, 'KALECİK'),
(69, 6, 'KAZAN'),
(70, 6, 'KEÇİÖREN'),
(71, 6, 'KIZILCAHAMAM'),
(72, 6, 'MAMAK'),
(73, 6, 'NALLIHAN'),
(74, 6, 'POLATLI'),
(75, 6, 'PURSAKLAR'),
(77, 6, 'ŞEREFLİKOÇHİSAR'),
(76, 6, 'SİNCAN'),
(78, 6, 'YENİMAHALLE'),
(79, 7, 'AKSEKİ'),
(80, 7, 'AKSU'),
(81, 7, 'ALANYA'),
(82, 7, 'DEMRE'),
(83, 7, 'DÖŞEMEALTI'),
(84, 7, 'ELMALI'),
(85, 7, 'FİNİKE'),
(86, 7, 'GAZİPAŞA'),
(87, 7, 'GÜNDOĞMUŞ'),
(88, 7, 'İBRADI'),
(89, 7, 'KAŞ'),
(90, 7, 'KEMER'),
(91, 7, 'KEPEZ'),
(92, 7, 'KONYAALTI'),
(93, 7, 'KORKUTELİ'),
(94, 7, 'KUMLUCA'),
(95, 7, 'MANAVGAT'),
(96, 7, 'MURATPAŞA'),
(97, 7, 'SERİK'),
(98, 8, 'ARDANUÇ'),
(99, 8, 'ARHAVİ'),
(100, 8, 'BORÇKA'),
(101, 8, 'HOPA'),
(102, 8, 'MURGUL'),
(103, 8, 'ŞAVŞAT'),
(104, 8, 'YUSUFELİ'),
(105, 9, 'BOZDOĞAN'),
(106, 9, 'BUHARKENT'),
(107, 9, 'ÇİNE'),
(108, 9, 'DİDİM'),
(893, 9, 'EFELER'),
(109, 9, 'GERMENCİK'),
(110, 9, 'İNCİRLİOVA'),
(111, 9, 'KARACASU'),
(112, 9, 'KARPUZLU'),
(113, 9, 'KOÇARLI'),
(114, 9, 'KÖŞK'),
(115, 9, 'KUŞADASI'),
(116, 9, 'KUYUCAK'),
(117, 9, 'NAZİLLİ'),
(118, 9, 'SÖKE'),
(119, 9, 'SULTANHİSAR'),
(120, 9, 'YENİPAZAR'),
(895, 10, 'ALTIEYLÜL'),
(121, 10, 'AYVALIK'),
(122, 10, 'BALYA'),
(123, 10, 'BANDIRMA'),
(124, 10, 'BİGADİÇ'),
(125, 10, 'BURHANİYE'),
(126, 10, 'DURSUNBEY'),
(127, 10, 'EDREMİT'),
(128, 10, 'ERDEK'),
(129, 10, 'GÖMEÇ'),
(130, 10, 'GÖNEN'),
(131, 10, 'HAVRAN'),
(132, 10, 'İVRİNDİ'),
(894, 10, 'KARESİ'),
(133, 10, 'KEPSUT'),
(134, 10, 'MANYAS'),
(135, 10, 'MARMARA'),
(136, 10, 'SAVAŞTEPE'),
(137, 10, 'SINDIRGI'),
(138, 10, 'SUSURLUK'),
(139, 11, 'BOZÜYÜK'),
(140, 11, 'GÖLPAZARI'),
(141, 11, 'İNHİSAR'),
(142, 11, 'OSMANELİ'),
(143, 11, 'PAZARYERİ'),
(144, 11, 'SÖĞÜT'),
(145, 11, 'YENİPAZAR'),
(146, 12, 'ADAKLI'),
(147, 12, 'GENÇ'),
(148, 12, 'KARLIOVA'),
(149, 12, 'KİĞI'),
(150, 12, 'SOLHAN'),
(151, 12, 'YAYLADERE'),
(152, 12, 'YEDİSU'),
(153, 13, 'ADİLCEVAZ'),
(154, 13, 'AHLAT'),
(155, 13, 'GÜROYMAK'),
(156, 13, 'HİZAN'),
(157, 13, 'MUTKİ'),
(158, 13, 'TATVAN'),
(159, 14, 'DÖRTDİVAN'),
(160, 14, 'GEREDE'),
(161, 14, 'GÖYNÜK'),
(162, 14, 'KIBRISCIK'),
(163, 14, 'MENGEN'),
(164, 14, 'MUDURNU'),
(165, 14, 'SEBEN'),
(166, 14, 'YENİÇAĞA'),
(167, 15, 'AĞLASUN'),
(168, 15, 'ALTINYAYLA'),
(169, 15, 'BUCAK'),
(170, 15, 'ÇAVDIR'),
(171, 15, 'ÇELTİKÇİ'),
(172, 15, 'GÖLHİSAR'),
(173, 15, 'KARAMANLI'),
(174, 15, 'KEMER'),
(175, 15, 'TEFENNİ'),
(176, 15, 'YEŞİLOVA'),
(177, 16, 'BÜYÜKORHAN'),
(178, 16, 'GEMLİK'),
(179, 16, 'GÜRSU'),
(180, 16, 'HARMANCIK'),
(181, 16, 'İNEGÖL'),
(182, 16, 'İZNİK'),
(183, 16, 'KARACABEY'),
(184, 16, 'KELES'),
(185, 16, 'KESTEL'),
(186, 16, 'MUDANYA'),
(187, 16, 'MUSTAFAKEMALPAŞA'),
(188, 16, 'NİLÜFER'),
(189, 16, 'ORHANELİ'),
(190, 16, 'ORHANGAZİ'),
(191, 16, 'OSMANGAZİ'),
(192, 16, 'YENİŞEHİR'),
(193, 16, 'YILDIRIM'),
(194, 17, 'AYVACIK'),
(195, 17, 'BAYRAMİÇ'),
(196, 17, 'BİGA'),
(197, 17, 'BOZCAADA'),
(198, 17, 'ÇAN'),
(199, 17, 'ECEABAT'),
(200, 17, 'EZİNE'),
(201, 17, 'GELİBOLU'),
(202, 17, 'İMROZ'),
(203, 17, 'LAPSEKİ'),
(204, 17, 'YENİCE'),
(205, 18, 'ATKARACALAR'),
(206, 18, 'BAYRAMÖREN'),
(207, 18, 'ÇERKEŞ'),
(208, 18, 'ELDİVAN'),
(209, 18, 'ILGAZ'),
(210, 18, 'KIZILIRMAK'),
(211, 18, 'KORGUN'),
(212, 18, 'KURŞUNLU'),
(213, 18, 'ORTA'),
(214, 18, 'ŞABANÖZÜ'),
(215, 18, 'YAPRAKLI'),
(216, 19, 'ALACA'),
(217, 19, 'BAYAT'),
(218, 19, 'BOĞAZKALE'),
(219, 19, 'DODURGA'),
(220, 19, 'İSKİLİP'),
(221, 19, 'KARGI'),
(222, 19, 'LAÇİN'),
(223, 19, 'MECİTÖZÜ'),
(224, 19, 'OĞUZLAR'),
(225, 19, 'ORTAKÖY'),
(226, 19, 'OSMANCIK'),
(227, 19, 'SUNGURLU'),
(228, 19, 'UĞURLUDAĞ'),
(229, 20, 'ACIPAYAM'),
(231, 20, 'BABADAĞ'),
(232, 20, 'BAKLAN'),
(233, 20, 'BEKİLLİ'),
(234, 20, 'BEYAĞAÇ'),
(235, 20, 'BOZKURT'),
(236, 20, 'BULDAN'),
(237, 20, 'ÇAL'),
(238, 20, 'ÇAMELİ'),
(239, 20, 'ÇARDAK'),
(240, 20, 'ÇİVRİL'),
(241, 20, 'GÜNEY'),
(242, 20, 'HONAZ'),
(243, 20, 'KALE'),
(896, 20, 'MERKEZEFENDİ'),
(230, 20, 'PAMUKKALE'),
(244, 20, 'SARAYKÖY'),
(245, 20, 'SERİNHİSAR'),
(246, 20, 'TAVAS'),
(247, 21, 'BAĞLAR'),
(248, 21, 'BİSMİL'),
(249, 21, 'ÇERMİK'),
(250, 21, 'ÇINAR'),
(251, 21, 'ÇÜNGÜŞ'),
(252, 21, 'DİCLE'),
(253, 21, 'EĞİL'),
(254, 21, 'ERGANİ'),
(255, 21, 'HANİ'),
(256, 21, 'HAZRO'),
(257, 21, 'KAYAPINAR'),
(258, 21, 'KOCAKÖY'),
(259, 21, 'KULP'),
(260, 21, 'LİCE'),
(261, 21, 'SİLVAN'),
(262, 21, 'SUR'),
(263, 21, 'YENİŞEHİR'),
(264, 22, 'ENEZ'),
(265, 22, 'HAVSA'),
(266, 22, 'İPSALA'),
(267, 22, 'KEŞAN'),
(268, 22, 'LALAPAŞA'),
(269, 22, 'MERİÇ'),
(270, 22, 'SÜLOĞLU'),
(271, 22, 'UZUNKÖPRÜ'),
(272, 23, 'AĞIN'),
(273, 23, 'ALACAKAYA'),
(274, 23, 'ARICAK'),
(275, 23, 'BASKİL'),
(276, 23, 'KARAKOÇAN'),
(277, 23, 'KEBAN'),
(278, 23, 'KOVANCILAR'),
(279, 23, 'MADEN'),
(280, 23, 'PALU'),
(281, 23, 'SİVRİCE'),
(282, 24, 'ÇAYIRLI'),
(283, 24, 'İLİÇ'),
(284, 24, 'KEMAH'),
(285, 24, 'KEMALİYE'),
(286, 24, 'OTLUKBELİ'),
(287, 24, 'REFAHİYE'),
(288, 24, 'TERCAN'),
(289, 24, 'ÜZÜMLÜ'),
(290, 25, 'AŞKALE'),
(291, 25, 'AZİZİYE'),
(292, 25, 'ÇAT'),
(293, 25, 'HINIS'),
(294, 25, 'HORASAN'),
(295, 25, 'İSPİR'),
(296, 25, 'KARAÇOBAN'),
(297, 25, 'KARAYAZI'),
(298, 25, 'KÖPRÜKÖY'),
(299, 25, 'NARMAN'),
(300, 25, 'OLTU'),
(301, 25, 'OLUR'),
(302, 25, 'PALANDÖKEN'),
(303, 25, 'PASİNLER'),
(304, 25, 'PAZARYOLU'),
(305, 25, 'ŞENKAYA'),
(306, 25, 'TEKMAN'),
(307, 25, 'TORTUM'),
(308, 25, 'UZUNDERE'),
(309, 25, 'YAKUTİYE'),
(310, 26, 'ALPU'),
(311, 26, 'BEYLİKOVA'),
(312, 26, 'ÇİFTELER'),
(313, 26, 'GÜNYÜZÜ'),
(314, 26, 'HAN'),
(315, 26, 'İNÖNÜ'),
(316, 26, 'MAHMUDİYE'),
(317, 26, 'MİHALGAZİ'),
(318, 26, 'MİHALIÇÇIK'),
(319, 26, 'ODUNPAZARI'),
(320, 26, 'SARICAKAYA'),
(321, 26, 'SEYİTGAZİ'),
(322, 26, 'SİVRİHİSAR'),
(323, 26, 'TEPEBAŞI'),
(324, 27, 'ARABAN'),
(325, 27, 'İSLAHİYE'),
(326, 27, 'KARKAMIŞ'),
(327, 27, 'NİZİP'),
(328, 27, 'NURDAĞI'),
(329, 27, 'OĞUZELİ'),
(330, 27, 'ŞAHİNBEY'),
(331, 27, 'ŞEHİTKAMİL'),
(332, 27, 'YAVUZELİ'),
(333, 28, 'ALUCRA'),
(334, 28, 'BULANCAK'),
(335, 28, 'ÇAMOLUK'),
(336, 28, 'ÇANAKÇI'),
(337, 28, 'DERELİ'),
(338, 28, 'DOĞANKENT'),
(339, 28, 'ESPİYE'),
(340, 28, 'EYNESİL'),
(341, 28, 'GÖRELE'),
(342, 28, 'GÜCE'),
(343, 28, 'KEŞAP'),
(344, 28, 'PİRAZİZ'),
(345, 28, 'ŞEBİNKARAHİSAR'),
(346, 28, 'TİREBOLU'),
(347, 28, 'YAĞLIDERE'),
(348, 29, 'KELKİT'),
(349, 29, 'KÖSE'),
(350, 29, 'KÜRTÜN'),
(351, 29, 'ŞİRAN'),
(352, 29, 'TORUL'),
(353, 30, 'ÇUKURCA'),
(354, 30, 'ŞEMDİNLİ'),
(355, 30, 'YÜKSEKOVA'),
(356, 31, 'ALTINÖZÜ'),
(897, 31, 'ANTAKYA'),
(899, 31, 'ARSUZ'),
(357, 31, 'BELEN'),
(898, 31, 'DEFNE'),
(358, 31, 'DÖRTYOL'),
(359, 31, 'ERZİN'),
(360, 31, 'HASSA'),
(361, 31, 'İSKENDERUN'),
(362, 31, 'KIRIKHAN'),
(363, 31, 'KUMLU'),
(900, 31, 'PAYAS'),
(364, 31, 'REYHANLI'),
(365, 31, 'SAMANDAĞ'),
(366, 31, 'YAYLADAĞI'),
(367, 32, 'AKSU'),
(368, 32, 'ATABEY'),
(369, 32, 'EĞİRDİR'),
(370, 32, 'GELENDOST'),
(371, 32, 'GÖNEN'),
(372, 32, 'KEÇİBORLU'),
(375, 32, 'ŞARKİKARAAĞAÇ'),
(373, 32, 'SENİRKENT'),
(374, 32, 'SÜTÇÜLER'),
(376, 32, 'ULUBORLU'),
(377, 32, 'YALVAÇ'),
(378, 32, 'YENİŞARBADEMLİ'),
(379, 33, 'AKDENİZ'),
(380, 33, 'ANAMUR'),
(381, 33, 'AYDINCIK'),
(382, 33, 'BOZYAZI'),
(383, 33, 'ÇAMLIYAYLA'),
(384, 33, 'ERDEMLİ'),
(385, 33, 'GÜLNAR'),
(386, 33, 'MEZİTLİ'),
(387, 33, 'MUT'),
(388, 33, 'SİLİFKE'),
(389, 33, 'TARSUS'),
(390, 33, 'TOROSLAR'),
(391, 33, 'YENİŞEHİR'),
(392, 34, 'ADALAR'),
(393, 34, 'ARNAVUTKÖY'),
(394, 34, 'ATAŞEHİR'),
(395, 34, 'AVCILAR'),
(396, 34, 'BAĞCILAR'),
(397, 34, 'BAHÇELİEVLER'),
(398, 34, 'BAKIRKÖY'),
(399, 34, 'BAŞAKŞEHİR'),
(400, 34, 'BAYRAMPAŞA'),
(401, 34, 'BEŞİKTAŞ'),
(402, 34, 'BEYKOZ'),
(403, 34, 'BEYLİKDÜZÜ'),
(404, 34, 'BEYOĞLU'),
(405, 34, 'BÜYÜKÇEKMECE'),
(406, 34, 'ÇATALCA'),
(407, 34, 'ÇEKMEKÖY'),
(408, 34, 'ESENLER'),
(409, 34, 'ESENYURT'),
(410, 34, 'EYÜP'),
(411, 34, 'FATİH'),
(412, 34, 'GAZİOSMANPAŞA'),
(413, 34, 'GÜNGÖREN'),
(414, 34, 'KADIKÖY'),
(415, 34, 'KAĞITHANE'),
(416, 34, 'KARTAL'),
(417, 34, 'KÜÇÜKÇEKMECE'),
(418, 34, 'MALTEPE'),
(419, 34, 'PENDİK'),
(420, 34, 'SANCAKTEPE'),
(421, 34, 'SARIYER'),
(425, 34, 'ŞİLE'),
(422, 34, 'SİLİVRİ'),
(426, 34, 'ŞİŞLİ'),
(423, 34, 'SULTANBEYLİ'),
(424, 34, 'SULTANGAZİ'),
(427, 34, 'TUZLA'),
(428, 34, 'ÜMRANİYE'),
(429, 34, 'ÜSKÜDAR'),
(430, 34, 'ZEYTİNBURNU'),
(431, 35, 'ALİAĞA'),
(432, 35, 'BALÇOVA'),
(433, 35, 'BAYINDIR'),
(434, 35, 'BAYRAKLI'),
(435, 35, 'BERGAMA'),
(436, 35, 'BEYDAĞ'),
(437, 35, 'BORNOVA'),
(438, 35, 'BUCA'),
(439, 35, 'ÇEŞME'),
(440, 35, 'ÇİĞLİ'),
(441, 35, 'DİKİLİ'),
(442, 35, 'FOÇA'),
(443, 35, 'GAZİEMİR'),
(444, 35, 'GÜZELBAHÇE'),
(445, 35, 'KARABAĞLAR'),
(446, 35, 'KARABURUN'),
(447, 35, 'KARŞIYAKA'),
(448, 35, 'KEMALPAŞA'),
(449, 35, 'KINIK'),
(450, 35, 'KİRAZ'),
(451, 35, 'KONAK'),
(452, 35, 'MENDERES'),
(453, 35, 'MENEMEN'),
(454, 35, 'NARLIDERE'),
(455, 35, 'ÖDEMİŞ'),
(456, 35, 'SEFERİHİSAR'),
(457, 35, 'SELÇUK'),
(458, 35, 'TİRE'),
(459, 35, 'TORBALI'),
(460, 35, 'URLA'),
(461, 36, 'AKYAKA'),
(462, 36, 'ARPAÇAY'),
(463, 36, 'DİGOR'),
(464, 36, 'KAĞIZMAN'),
(465, 36, 'SARIKAMIŞ'),
(466, 36, 'SELİM'),
(467, 36, 'SUSUZ'),
(468, 37, 'ABANA'),
(469, 37, 'AĞLI'),
(470, 37, 'ARAÇ'),
(471, 37, 'AZDAVAY'),
(472, 37, 'BOZKURT'),
(474, 37, 'ÇATALZEYTİN'),
(473, 37, 'CİDE'),
(475, 37, 'DADAY'),
(476, 37, 'DEVREKANİ'),
(477, 37, 'DOĞANYURT'),
(478, 37, 'HANÖNÜ'),
(479, 37, 'İHSANGAZİ'),
(480, 37, 'İNEBOLU'),
(481, 37, 'KÜRE'),
(482, 37, 'PINARBAŞI'),
(484, 37, 'ŞENPAZAR'),
(483, 37, 'SEYDİLER'),
(485, 37, 'TAŞKÖPRÜ'),
(486, 37, 'TOSYA'),
(487, 38, 'AKKIŞLA'),
(488, 38, 'BÜNYAN'),
(489, 38, 'DEVELİ'),
(490, 38, 'FELAHİYE'),
(491, 38, 'HACILAR'),
(492, 38, 'İNCESU'),
(493, 38, 'KOCASİNAN'),
(494, 38, 'MELİKGAZİ'),
(495, 38, 'ÖZVATAN'),
(496, 38, 'PINARBAŞI'),
(497, 38, 'SARIOĞLAN'),
(498, 38, 'SARIZ'),
(499, 38, 'TALAS'),
(500, 38, 'TOMARZA'),
(501, 38, 'YAHYALI'),
(502, 38, 'YEŞİLHİSAR'),
(503, 39, 'BABAESKİ'),
(504, 39, 'DEMİRKÖY'),
(505, 39, 'KOFÇAZ'),
(506, 39, 'LÜLEBURGAZ'),
(507, 39, 'PEHLİVANKÖY'),
(508, 39, 'PINARHİSAR'),
(509, 39, 'VİZE'),
(510, 40, 'AKÇAKENT'),
(511, 40, 'AKPINAR'),
(512, 40, 'BOZTEPE'),
(513, 40, 'ÇİÇEKDAĞI'),
(514, 40, 'KAMAN'),
(515, 40, 'MUCUR'),
(516, 41, 'BAŞİSKELE'),
(517, 41, 'ÇAYIROVA'),
(518, 41, 'DARICA'),
(519, 41, 'DERİNCE'),
(520, 41, 'DİLOVASI'),
(521, 41, 'GEBZE'),
(522, 41, 'GÖLCÜK'),
(523, 41, 'İZMİT'),
(524, 41, 'KANDIRA'),
(525, 41, 'KARAMÜRSEL'),
(526, 41, 'KARTEPE'),
(527, 41, 'KÖRFEZ'),
(528, 42, 'AHIRLI'),
(529, 42, 'AKÖREN'),
(530, 42, 'AKŞEHİR'),
(531, 42, 'ALTINEKİN'),
(532, 42, 'BEYŞEHİR'),
(533, 42, 'BOZKIR'),
(535, 42, 'ÇELTİK'),
(534, 42, 'CİHANBEYLİ'),
(536, 42, 'ÇUMRA'),
(537, 42, 'DERBENT'),
(538, 42, 'DEREBUCAK'),
(539, 42, 'DOĞANHİSAR'),
(540, 42, 'EMİRGAZİ'),
(541, 42, 'EREĞLİ'),
(542, 42, 'GÜNEYSINIR'),
(543, 42, 'HADİM'),
(544, 42, 'HALKAPINAR'),
(545, 42, 'HÜYÜK'),
(546, 42, 'ILGIN'),
(547, 42, 'KADINHANI'),
(548, 42, 'KARAPINAR'),
(549, 42, 'KARATAY'),
(550, 42, 'KULU'),
(551, 42, 'MERAM'),
(552, 42, 'SARAYÖNÜ'),
(553, 42, 'SELÇUKLU'),
(554, 42, 'SEYDİŞEHİR'),
(555, 42, 'TAŞKENT'),
(556, 42, 'TUZLUKÇU'),
(557, 42, 'YALIHÜYÜK'),
(558, 42, 'YUNAK'),
(559, 43, 'ALTINTAŞ'),
(560, 43, 'ASLANAPA'),
(561, 43, 'ÇAVDARHİSAR'),
(562, 43, 'DOMANİÇ'),
(563, 43, 'DUMLUPINAR'),
(564, 43, 'EMET'),
(565, 43, 'GEDİZ'),
(566, 43, 'HİSARCIK'),
(567, 43, 'PAZARLAR'),
(569, 43, 'ŞAPHANE'),
(568, 43, 'SİMAV'),
(570, 43, 'TAVŞANLI'),
(571, 44, 'AKÇADAĞ'),
(572, 44, 'ARAPGİR'),
(573, 44, 'ARGUVAN'),
(574, 44, 'BATTALGAZİ'),
(575, 44, 'DARENDE'),
(576, 44, 'DOĞANŞEHİR'),
(577, 44, 'DOĞANYOL'),
(578, 44, 'HEKİMHAN'),
(579, 44, 'KALE'),
(580, 44, 'KULUNCAK'),
(581, 44, 'PÜTÜRGE'),
(582, 44, 'YAZIHAN'),
(583, 44, 'YEŞİLYURT'),
(584, 45, 'AHMETLİ'),
(585, 45, 'AKHİSAR'),
(586, 45, 'ALAŞEHİR'),
(587, 45, 'DEMİRCİ'),
(588, 45, 'GÖLMARMARA'),
(589, 45, 'GÖRDES'),
(590, 45, 'KIRKAĞAÇ'),
(591, 45, 'KÖPRÜBAŞI'),
(592, 45, 'KULA'),
(593, 45, 'SALİHLİ'),
(594, 45, 'SARIGÖL'),
(595, 45, 'SARUHANLI'),
(901, 45, 'ŞEHZADELER'),
(596, 45, 'SELENDİ'),
(597, 45, 'SOMA'),
(598, 45, 'TURGUTLU'),
(902, 45, 'YUNUSEMRE'),
(599, 46, 'AFŞİN'),
(600, 46, 'ANDIRIN'),
(601, 46, 'ÇAĞLAYANCERİT'),
(903, 46, 'DULKADİROĞLU'),
(602, 46, 'EKİNÖZÜ'),
(603, 46, 'ELBİSTAN'),
(604, 46, 'GÖKSUN'),
(605, 46, 'NURHAK'),
(904, 46, 'ONİKİŞUBAT'),
(606, 46, 'PAZARCIK'),
(607, 46, 'TÜRKOĞLU'),
(905, 47, 'ARTUKLU'),
(608, 47, 'DARGEÇİT'),
(609, 47, 'DERİK'),
(610, 47, 'KIZILTEPE'),
(611, 47, 'MAZIDAĞI'),
(612, 47, 'MİDYAT'),
(613, 47, 'NUSAYBİN'),
(614, 47, 'ÖMERLİ'),
(615, 47, 'SAVUR'),
(616, 47, 'YEŞİLLİ'),
(617, 48, 'BODRUM'),
(618, 48, 'DALAMAN'),
(619, 48, 'DATÇA'),
(620, 48, 'FETHİYE'),
(621, 48, 'KAVAKLIDERE'),
(622, 48, 'KÖYCEĞİZ'),
(623, 48, 'MARMARİS'),
(906, 48, 'MENTEŞE'),
(624, 48, 'MİLAS'),
(625, 48, 'ORTACA'),
(907, 48, 'SEYDİKEMER'),
(626, 48, 'ULA'),
(627, 48, 'YATAĞAN'),
(628, 49, 'BULANIK'),
(629, 49, 'HASKÖY'),
(630, 49, 'KORKUT'),
(631, 49, 'MALAZGİRT'),
(632, 49, 'VARTO'),
(633, 50, 'ACIGÖL'),
(634, 50, 'AVANOS'),
(635, 50, 'DERİNKUYU'),
(636, 50, 'GÜLŞEHİR'),
(637, 50, 'HACIBEKTAŞ'),
(638, 50, 'KOZAKLI'),
(639, 50, 'ÜRGÜP'),
(640, 51, 'ALTUNHİSAR'),
(641, 51, 'BOR'),
(642, 51, 'ÇAMARDI'),
(643, 51, 'ÇİFTLİK'),
(644, 51, 'ULUKIŞLA'),
(645, 52, 'AKKUŞ'),
(646, 52, 'AYBASTI'),
(647, 52, 'ÇAMAŞ'),
(648, 52, 'ÇATALPINAR'),
(649, 52, 'ÇAYBAŞI'),
(650, 52, 'FATSA'),
(651, 52, 'GÖLKÖY'),
(652, 52, 'GÜLYALI'),
(653, 52, 'GÜRGENTEPE'),
(654, 52, 'İKİZCE'),
(655, 52, 'KABADÜZ'),
(656, 52, 'KABATAŞ'),
(657, 52, 'KORGAN'),
(658, 52, 'KUMRU'),
(659, 52, 'MESUDİYE'),
(660, 52, 'PERŞEMBE'),
(661, 52, 'ULUBEY'),
(662, 52, 'ÜNYE'),
(663, 53, 'ARDEŞEN'),
(664, 53, 'ÇAMLIHEMŞİN'),
(665, 53, 'ÇAYELİ'),
(666, 53, 'DEREPAZARI'),
(667, 53, 'FINDIKLI'),
(668, 53, 'GÜNEYSU'),
(669, 53, 'HEMŞİN'),
(670, 53, 'İKİZDERE'),
(671, 53, 'İYİDERE'),
(672, 53, 'KALKANDERE'),
(673, 53, 'PAZAR'),
(674, 54, 'ADAPAZARI'),
(675, 54, 'AKYAZI'),
(676, 54, 'ARİFİYE'),
(677, 54, 'ERENLER'),
(678, 54, 'FERİZLİ'),
(679, 54, 'GEYVE'),
(680, 54, 'HENDEK'),
(681, 54, 'KARAPÜRÇEK'),
(682, 54, 'KARASU'),
(683, 54, 'KAYNARCA'),
(684, 54, 'KOCAALİ'),
(685, 54, 'PAMUKOVA'),
(686, 54, 'SAPANCA'),
(687, 54, 'SERDİVAN'),
(688, 54, 'SÖĞÜTLÜ'),
(689, 54, 'TARAKLI'),
(691, 55, 'ALAÇAM'),
(692, 55, 'ASARCIK'),
(693, 55, 'ATAKUM'),
(694, 55, 'AYVACIK'),
(695, 55, 'BAFRA'),
(696, 55, 'CANİK'),
(697, 55, 'ÇARŞAMBA'),
(698, 55, 'HAVZA'),
(699, 55, 'İLKADIM'),
(700, 55, 'KAVAK'),
(701, 55, 'LADİK'),
(690, 55, 'ONDOKUZMAYIS'),
(702, 55, 'SALIPAZARI'),
(703, 55, 'TEKKEKÖY'),
(704, 55, 'TERME'),
(705, 55, 'VEZİRKÖPRÜ'),
(706, 55, 'YAKAKENT'),
(707, 56, 'AYDINLAR'),
(713, 57, 'AYANCIK'),
(708, 57, 'BAYKAN'),
(721, 58, 'AKINCILAR'),
(722, 58, 'ALTINYAYLA'),
(714, 58, 'BOYABAT'),
(723, 58, 'DİVRİĞİ'),
(724, 58, 'DOĞANŞAR'),
(709, 58, 'ERUH'),
(725, 58, 'GEMEREK'),
(726, 58, 'GÖLOVA'),
(727, 58, 'GÜRÜN'),
(728, 58, 'HAFİK'),
(729, 58, 'İMRANLI'),
(730, 58, 'KANGAL'),
(731, 58, 'KOYULHİSAR'),
(733, 58, 'ŞARKIŞLA'),
(732, 58, 'SUŞEHRİ'),
(734, 58, 'ULAŞ'),
(735, 58, 'YILDIZELİ'),
(736, 58, 'ZARA'),
(737, 59, 'ÇERKEZKÖY'),
(738, 59, 'ÇORLU'),
(715, 59, 'DİKMEN'),
(910, 59, 'ERGENE'),
(739, 59, 'HAYRABOLU'),
(909, 59, 'KAPAKLI'),
(710, 59, 'KURTALAN'),
(740, 59, 'MALKARA'),
(741, 59, 'MARMARAEREĞLİSİ'),
(742, 59, 'MURATLI'),
(743, 59, 'SARAY'),
(744, 59, 'ŞARKÖY'),
(908, 59, 'SÜLEYMANPAŞA'),
(745, 60, 'ALMUS'),
(746, 60, 'ARTOVA'),
(747, 60, 'BAŞÇİFTLİK'),
(716, 60, 'DURAĞAN'),
(748, 60, 'ERBAA'),
(749, 60, 'NİKSAR'),
(750, 60, 'PAZAR'),
(711, 60, 'PERVARİ'),
(751, 60, 'REŞADİYE'),
(752, 60, 'SULUSARAY'),
(753, 60, 'TURHAL'),
(754, 60, 'YEŞİLYURT'),
(755, 60, 'ZİLE'),
(756, 61, 'AKÇAABAT'),
(757, 61, 'ARAKLI'),
(758, 61, 'ARSİN'),
(759, 61, 'BEŞİKDÜZÜ'),
(760, 61, 'ÇARŞIBAŞI'),
(761, 61, 'ÇAYKARA'),
(762, 61, 'DERNEKPAZARI'),
(763, 61, 'DÜZKÖY'),
(717, 61, 'ERFELEK'),
(764, 61, 'HAYRAT'),
(765, 61, 'KÖPRÜBAŞI'),
(766, 61, 'MAÇKA'),
(767, 61, 'OF'),
(911, 61, 'ORTAHİSAR'),
(769, 61, 'ŞALPAZARI'),
(712, 61, 'ŞİRVAN'),
(768, 61, 'SÜRMENE'),
(770, 61, 'TONYA'),
(771, 61, 'VAKFIKEBİR'),
(772, 61, 'YOMRA'),
(773, 62, 'ÇEMİŞGEZEK'),
(718, 62, 'GERZE'),
(774, 62, 'HOZAT'),
(775, 62, 'MAZGİRT'),
(776, 62, 'NAZIMİYE'),
(777, 62, 'OVACIK'),
(778, 62, 'PERTEK'),
(779, 62, 'PÜLÜMÜR'),
(780, 63, 'AKÇAKALE'),
(781, 63, 'BİRECİK'),
(782, 63, 'BOZOVA'),
(783, 63, 'CEYLANPINAR'),
(912, 63, 'EYYÜBİYE'),
(784, 63, 'HALFETİ'),
(913, 63, 'HALİLİYE'),
(785, 63, 'HARRAN'),
(786, 63, 'HİLVAN'),
(914, 63, 'KARAKÖPRÜ'),
(719, 63, 'SARAYDÜZÜ'),
(787, 63, 'SİVEREK'),
(788, 63, 'SURUÇ'),
(789, 63, 'VİRANŞEHİR'),
(790, 64, 'BANAZ'),
(791, 64, 'EŞME'),
(792, 64, 'KARAHALLI'),
(793, 64, 'SİVASLI'),
(720, 64, 'TÜRKELİ'),
(794, 64, 'ULUBEY'),
(795, 65, 'BAHÇESARAY'),
(796, 65, 'BAŞKALE'),
(797, 65, 'ÇALDIRAN'),
(798, 65, 'ÇATAK'),
(799, 65, 'EDREMİT'),
(800, 65, 'ERCİŞ'),
(801, 65, 'GEVAŞ'),
(802, 65, 'GÜRPINAR'),
(916, 65, 'İPEKYOLU'),
(803, 65, 'MURADİYE'),
(804, 65, 'ÖZALP'),
(805, 65, 'SARAY'),
(915, 65, 'TUŞBA'),
(806, 66, 'AKDAĞMADENİ'),
(807, 66, 'AYDINCIK'),
(808, 66, 'BOĞAZLIYAN'),
(809, 66, 'ÇANDIR'),
(810, 66, 'ÇAYIRALAN'),
(811, 66, 'ÇEKEREK'),
(812, 66, 'KADIŞEHRİ'),
(813, 66, 'SARAYKENT'),
(814, 66, 'SARIKAYA'),
(816, 66, 'ŞEFAATLİ'),
(815, 66, 'SORGUN'),
(817, 66, 'YENİFAKILI'),
(818, 66, 'YERKÖY'),
(819, 67, 'ALAPLI'),
(820, 67, 'ÇAYCUMA'),
(821, 67, 'DEVREK'),
(822, 67, 'EREĞLİ'),
(823, 67, 'GÖKÇEBEY'),
(917, 67, 'KİLİMLİ'),
(824, 68, 'AĞAÇÖREN'),
(825, 68, 'ESKİL'),
(826, 68, 'GÜLAĞAÇ'),
(827, 68, 'GÜZELYURT'),
(828, 68, 'ORTAKÖY'),
(829, 68, 'SARIYAHŞİ'),
(830, 69, 'AYDINTEPE'),
(831, 69, 'DEMİRÖZÜ'),
(832, 70, 'AYRANCI'),
(833, 70, 'BAŞYAYLA'),
(834, 70, 'ERMENEK'),
(835, 70, 'KAZIMKARABEKİR'),
(836, 70, 'SARIVELİLER'),
(837, 71, 'BAHŞİLİ'),
(838, 71, 'BALIŞEYH'),
(839, 71, 'ÇELEBİ'),
(840, 71, 'DELİCE'),
(841, 71, 'KARAKEÇİLİ'),
(842, 71, 'KESKİN'),
(843, 71, 'SULAKYURT'),
(844, 71, 'YAHŞİHAN'),
(845, 72, 'BEŞİRİ'),
(846, 72, 'GERCÜŞ'),
(847, 72, 'HASANKEYF'),
(848, 72, 'KOZLUK'),
(849, 72, 'SASON'),
(850, 73, 'BEYTÜŞŞEBAP'),
(851, 73, 'CİZRE'),
(852, 73, 'GÜÇLÜKONAK'),
(853, 73, 'İDİL'),
(854, 73, 'SİLOPİ'),
(855, 73, 'ULUDERE'),
(856, 74, 'AMASRA'),
(857, 74, 'KURUCAŞİLE'),
(858, 74, 'ULUS'),
(859, 75, 'ÇILDIR'),
(860, 75, 'DAMAL'),
(861, 75, 'GÖLE'),
(862, 75, 'HANAK'),
(863, 75, 'POSOF'),
(864, 76, 'ARALIK'),
(865, 76, 'KARAKOYUNLU'),
(866, 76, 'TUZLUCA'),
(867, 77, 'ALTINOVA'),
(868, 77, 'ARMUTLU'),
(870, 77, 'ÇİFTLİKKÖY'),
(869, 77, 'ÇINARCIK'),
(871, 77, 'TERMAL'),
(872, 78, 'EFLANİ'),
(873, 78, 'ESKİPAZAR'),
(874, 78, 'OVACIK'),
(875, 78, 'SAFRANBOLU'),
(876, 78, 'YENİCE'),
(877, 79, 'ELBEYLİ'),
(878, 79, 'MUSABEYLİ'),
(879, 79, 'POLATELİ'),
(880, 80, 'BAHÇE'),
(881, 80, 'DÜZİÇİ'),
(882, 80, 'HASANBEYLİ'),
(883, 80, 'KADİRLİ'),
(884, 80, 'SUMBAS'),
(885, 80, 'TOPRAKKALE'),
(886, 81, 'AKÇAKOCA'),
(888, 81, 'ÇİLİMLİ'),
(887, 81, 'CUMAYERİ'),
(889, 81, 'GÖLYAKA'),
(890, 81, 'GÜMÜŞOVA'),
(891, 81, 'KAYNAŞLI'),
(892, 81, 'YIĞILCA');

-- --------------------------------------------------------

--
-- Tablo için tablo yapısı `region`
--

CREATE TABLE `region` (
  `id` int(11) NOT NULL,
  `name` varchar(64) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_turkish_ci;

--
-- Tablo döküm verisi `region`
--

INSERT INTO `region` (`id`, `name`) VALUES
(1, 'AKDENİZ'),
(2, 'DOĞU ANADOLU'),
(3, 'EGE'),
(7, 'GÜNEYDOĞU ANADOLU'),
(4, 'İÇ ANADOLU'),
(5, 'KARADENİZ'),
(6, 'MARMARA');

--
-- Dökümü yapılmış tablolar için indeksler
--

--
-- Tablo için indeksler `city`
--
ALTER TABLE `city`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `name` (`name`),
  ADD KEY `region_id` (`region_id`);

--
-- Tablo için indeksler `district`
--
ALTER TABLE `district`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `unique_multiple_columns_1` (`city_plate_id`,`name`);

--
-- Tablo için indeksler `region`
--
ALTER TABLE `region`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `name` (`name`);

--
-- Dökümü yapılmış tablolar için AUTO_INCREMENT değeri
--

--
-- Tablo için AUTO_INCREMENT değeri `district`
--
ALTER TABLE `district`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=918;

--
-- Tablo için AUTO_INCREMENT değeri `region`
--
ALTER TABLE `region`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=8;

--
-- Dökümü yapılmış tablolar için kısıtlamalar
--

--
-- Tablo kısıtlamaları `city`
--
ALTER TABLE `city`
  ADD CONSTRAINT `city_ibfk_1` FOREIGN KEY (`region_id`) REFERENCES `region` (`id`);

--
-- Tablo kısıtlamaları `district`
--
ALTER TABLE `district`
  ADD CONSTRAINT `district_ibfk_1` FOREIGN KEY (`city_plate_id`) REFERENCES `city` (`id`);
COMMIT;

/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
